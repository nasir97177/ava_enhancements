...
class Analytics(object):
		...
		self.columns = [
			{
				...
				"sales_by_channel_chart": 1 if self.filters.tree_type == "Customer Group" else 0,
	...
	def get_data(self):
		if self.filters.tree_type in ["Customer", "Supplier"]:
			...
			self.get_sales_transactions_based_on_customer_or_territory_group_warehouse()
	...
	def get_sales_transactions_based_on_items(self):
		...
		self.entries = frappe.db.sql("""
			...
			where s.name = i.parent and i.docstatus = 1 and s.company = %s and s.territory is not null 
			and s.{date_field} between %s and %s
		"""
		.format(date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type),
		(self.filters.company, self.filters.from_date, self.filters.to_date), as_dict=1)
		# frappe.msgprint(frappe.utils.cstr(len(self.entries)))
		# frappe.msgprint(frappe.utils.cstr(sum([d.value_field for d in self.entries])))

		self.entity_names = {}
		for d in self.entries:
			self.entity_names.setdefault(d.entity, d.entity_name)

	def get_sales_transactions_based_on_customer_or_territory_group_warehouse(self):
		if self.filters["value_quantity"] == 'Value':
			value_field = "base_net_total as value_field"
		else:
			value_field = "total_qty as value_field"
	...
	def get_groups(self):
		...
		if root and self.filters.tree_type == 'Warehouse':
		...
		self.group_entries = frappe.db.sql("""select name, lft, rgt , {parent} as parent
			from `tab{tree}` where 1=1 {cond} order by lft"""
		.format(tree=self.filters.tree_type, parent=parent, cond=cond), as_dict=1)
	...
	def get_chart_data(self):
		labels = datasets = []
		if self.filters.charts_based_on == 'Default':
			chart_type = "line"
		...
		if self.filters.charts_based_on == 'Sales by Region':
			chart_type = "pie"
			if self.filters.tree_type == "Warehouse":
				labels = [d.get('entity') for d in self.data if d.get('indent') == 2 and flt(d.get('total')) > 0]
				data = [d.get('total') for d in self.data if d.get('indent') == 2 and flt(d.get('total')) > 0]
				datasets.append({'values': data})

			else:
				frappe.throw("Set Warehouse in Tree Type")

		if self.filters.charts_based_on == 'Sales by Channel':
			chart_type = "pie"
			if self.filters.tree_type == "Customer Group":
				labels = [d.get('entity') for d in self.data if d.get('indent') == 1]
				data = [d.get('total') for d in self.data if d.get('indent') == 1]
				datasets.append({'values': data})

			else:
				frappe.throw("Set Customer Group in Tree Type")

		if self.filters.charts_based_on == 'Sales by Route Efficiency':
			chart_type = "bar"
			if self.filters.tree_type == "Territory":
				labels = [d.get('entity') for d in self.data if d.get('indent') == 3 and flt(d.get('total')) > 0]
				# frappe.msgprint(frappe.utils.cstr(labels))
				data = [d.get('total') for d in self.data if d.get('indent') == 3 and flt(d.get('total')) > 0]
				datasets.append({'values': data})

			else:
				frappe.throw("Set Territory in Tree Type")

		# if self.filters.charts_based_on == 'Channel Location Mix':
		# 	for d in self.data[0:5]:
		# 		frappe.msgprint(frappe.utils.cstr(d))
