...
class AccruedEndofService(AccountsController):
	def onload(self):
	...
	def set_payable_account(self):
		if not self.payable_account and not self.is_paid:
			self.payable_account = frappe.get_cached_value(
				"Company", self.company, "default_accrued_ticket_payable_account"
			)

	def set_expense_account(self):
		if not self.expense_account and not self.is_paid:
			self.expense_account = frappe.get_cached_value(
				"Company", self.company, "default_accrued_ticket_expense_account"
			)
	...
	def update_task_and_project(self):
		if self.task:
			...
			ExpenseClaim = frappe.qb.DocType("Accrued End of Service")
	...
	def validate_account_details(self):
		for data in self.expenses:
			if not data.cost_center:
				frappe.throw(
					_("Row {0}: {1} is required in the expenses table to book an Accrued End of Service.").format(
	...
	# 		# self.total_claimed_amount += flt(self.basic)
	# 		self.total_sanctioned_amount = flt((self.basic + (self.basic*.25) + (self.basic*.10) )/30*2.5)
	...
	# 	# )
	# 	self.grand_total = (
	# 		flt((self.basic + (self.basic*.25) + (self.basic*.10) )/30*2.5)
	# 		+ flt(self.total_taxes_and_charges)
	# 		- flt(self.total_advance_amount)
	# 	)
...
def update_reimbursed_amount(doc):
	...
	frappe.db.set_value("Accrued End of Service", doc.name, "total_amount_reimbursed", total_amount_reimbursed)
...
def make_bank_entry(dt, dn):
	...
	je.remark = "Payment against Accrued End of Service: " + dn
	...
	je.append(
		"accounts",
		{
			...
			"reference_type": "Accrued End of Service",
			"party_type": "Employee",
...
def get_expense_claim(
	...
	expense_claim = frappe.new_doc("Accrued End of Service")
...
def update_payment_for_expense_claim(doc, method=None):
	"""
	Updates payment/reimbursed amount in Accrued End of Service
	...
	for d in doc.get(payment_table):
		if d.get(doctype_field) == "Accrued End of Service" and d.reference_name:
			expense_claim = frappe.get_doc("Accrued End of Service", d.reference_name)
			if doc.docstatus == 2:
...
def validate_expense_claim_in_jv(doc, method=None):
	...
	for d in doc.accounts:
		if d.reference_type == "Accrued End of Service":
			outstanding_amt = get_outstanding_amount_for_claim(d.reference_name)
			if d.debit > outstanding_amt:
				frappe.throw(
					_(
						"Row No {0}: Amount cannot be greater than the Outstanding Amount against Accrued End of Service {1}. Outstanding Amount is {2}"
...
def make_expense_claim_for_delivery_trip(source_name, target_doc=None):
	doc = get_mapped_doc(
		"Delivery Trip",
		source_name,
		{"Delivery Trip": {"doctype": "Accrued End of Service", "field_map": {"name": "delivery_trip"}}},
		target_doc,
	)
