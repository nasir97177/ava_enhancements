...
# class EmployeeProvision(Document):
# 	pass
import frappe
...
class EmployeeProvision(AccountsController):
	def onload(self):
	...
	def validate_account_details(self):
		for data in self.expenses:
			if not data.cost_center:
				frappe.throw(_("Row {0}: {1} is required in the expenses table to book an Employee Provision.")
...
def update_reimbursed_amount(doc, amount):
	...
	frappe.db.set_value("Employee Provision", doc.name , "total_amount_reimbursed", doc.total_amount_reimbursed)

	doc.set_status()
	frappe.db.set_value("Employee Provision", doc.name , "status", doc.status)
...
def make_bank_entry(dt, dn):
	...
	je.remark = 'Payment against Employee Provision: ' + dn

	je.append("accounts", {
		"account": expense_claim.payable_account,
		"debit_in_account_currency": payable_amount,
		"reference_type": "Employee Provision",
		"party_type": "Employee",
...
def get_expense_claim_account(expense_claim_type, company):
	account = frappe.db.get_value("Employee Provision Account",
...
def get_expense_claim(
	...
	expense_claim = frappe.new_doc('Employee Provision')
