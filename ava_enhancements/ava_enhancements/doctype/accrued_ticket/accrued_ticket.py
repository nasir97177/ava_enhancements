...
class AccruedTicket(AccountsController):
	def onload(self):
	...
	def update_task_and_project(self):
		if self.task:
			...
			ExpenseClaim = frappe.qb.DocType("Accrued Ticket")
	...
	def validate_account_details(self):
		for data in self.expenses:
			if not data.cost_center:
				frappe.throw(
					_("Row {0}: {1} is required in the expenses table to book an Accrued Ticket.").format(
	...
	# 		self.total_sanctioned_amount = flt(self.total_ticket_allowance)/(12*flt(self.ticket_period))
	...
	def calculate_taxes(self):
		...
		# )
		# self.grand_total = (
		# 	flt(self.total_ticket_allowance)/(12*flt(self.ticket_period))
		# 	+ flt(self.total_taxes_and_charges)
		# 	- flt(self.total_advance_amount)
		# )
...
def update_reimbursed_amount(doc):
	...
	frappe.db.set_value("Accrued Ticket", doc.name, "total_amount_reimbursed", total_amount_reimbursed)
...
def make_bank_entry(dt, dn):
	...
	je.remark = "Payment against Accrued Ticket: " + dn
...
def get_expense_claim(
	...
	expense_claim = frappe.new_doc("Accrued Ticket")
...
def update_payment_for_expense_claim(doc, method=None):
	"""
	Updates payment/reimbursed amount in Accrued Ticket
	...
	for d in doc.get(payment_table):
		if d.get(doctype_field) == "Accrued Ticket" and d.reference_name:
			expense_claim = frappe.get_doc("Accrued Ticket", d.reference_name)
			if doc.docstatus == 2:
...
def validate_expense_claim_in_jv(doc, method=None):
	...
	for d in doc.accounts:
		if d.reference_type == "Accrued Ticket":
			outstanding_amt = get_outstanding_amount_for_claim(d.reference_name)
			if d.debit > outstanding_amt:
				frappe.throw(
					_(
						"Row No {0}: Amount cannot be greater than the Outstanding Amount against Accrued Ticket {1}. Outstanding Amount is {2}"
...
def make_expense_claim_for_delivery_trip(source_name, target_doc=None):
	doc = get_mapped_doc(
		"Delivery Trip",
		source_name,
		{"Delivery Trip": {"doctype": "Accrued Ticket", "field_map": {"name": "delivery_trip"}}},
		target_doc,
	)
