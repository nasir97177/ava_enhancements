...
// frappe.provide("irsaa_report.irsaa_report");
...
frappe.ui.form.on('Accrued Vacation', {
	...
	company: function(frm) {
		...
		for (var i = 0; i < expenses.length; i++) {
			...
			frappe.call({
				method: "irsaa_report.irsaa_report.doctype.accrued_ticket.accrued_ticket.get_accrued_ticket_account_and_cost_center",
				args: {
...
frappe.ui.form.on('Employee Provision Detail', {
	expense_type: function(frm, cdt, cdn) {
		...
		return frappe.call({
			method: "hr.doctype.expense_claim.expense_claim.get_expense_claim_account_and_cost_center",
			args: {
...
frappe.ui.form.on("Accrued Vacation", {
	...
	make_payment_entry: function(frm) {
		let method = "hr.overrides.employee_payment_entry.get_payment_entry_for_employee";
		if(frm.doc.__onload && frm.doc.__onload.make_payment_via_journal_entry) {
			method = "hr.doctype.expense_claim.expense_claim.make_bank_entry";
		}
		return frappe.call({
			method: method,
			args: {
